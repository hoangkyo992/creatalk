<template>
  <Dialog :visible="true" :style="{ width: '90vw', maxWidth: '480px', minWidth: '360px' }" :header="$t('RolesPage.Header')" :modal="true" :closable="false">
    <form v-focustrap @submit.prevent class="grid formgrid p-fluid">
      <div class="field mb-6 col-12">
        <label for="name" class="required">{{ $t("RolesPage.Name") }}</label>
      ...
      <div class="field mb-6 col-12">
        <label for="description" class="required">{{ $t("RolesPage.Description") }}</label>
        <InputText id="description" v-model="item.description" autocomplete="off" size="small" type="text" :invalid="v.item.description.$errors.length > 0" />
    ...
    <template #footer>
      <Button
        v-if="(!id && grantStore.hasPermission(`Administration.Roles`, `Create`)) || (id && grantStore.hasPermission(`Administration.Roles`, `Update`))"
        type="submit"
        icon="pi pi-save"
...
import type { UpdateRoleRequestDto } from "@/contracts/Roles";
...
const item = reactive<UpdateRoleRequestDto>({} as UpdateRoleRequestDto);

const validator = useAppValidation();
const rules = computed(() => ({
  item: {
    name: {
      required: helpers.withMessage($t("RolesPage.Validations.NameIsRequired"), required),
      maxLength: validator.maxLength(50)
    },
    description: {
      required: helpers.withMessage($t("RolesPage.Validations.DescriptionIsRequired"), required),
